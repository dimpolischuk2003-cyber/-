...
(function () {
  ...
  function initHero() {
    ...
    if (ph) {
      const mobileHero = next.speaker === 'noienko' ? 'assets/images/noienko-mobile.webp' : '';
      const setHeroPhoto = () => {
        const useMobile = window.matchMedia('(max-width: 640px)').matches && mobileHero;
        const src = useMobile ? mobileHero : window.V.speakerPhoto(next);
        if (ph.getAttribute('src') !== src) ph.src = src;
        ph.alt = sp.name;
        ph.classList.toggle('hero__photo--real', Boolean(useMobile));
      };
      setHeroPhoto();
      const mq = window.matchMedia('(max-width: 640px)');
      if (mq.addEventListener) mq.addEventListener('change', setHeroPhoto);
      else if (mq.addListener) mq.addListener(setHeroPhoto);
    }
  }
