...
(function () {
  const { D, qs, qsa, MONTHS, MONTHS_GEN, parseDate, daysUntil, daysLabel,
          plural, formatPrice, mainPrice, upcoming, trackEvent, courseUrl } = window.V;
  ...
  function initHero() {
    ...
    if (details) details.href = courseUrl(next);
    ...
    return `
      <article class="ccard">
        ...
        <h3 class="ccard__title">
          <a href="${esc(courseUrl(c))}">${esc(c.title)}</a>
        </h3>
        ...
        <div class="ccard__actions">
          ...
          <a class="btn btn--ghost btn--block" href="${esc(courseUrl(c))}">Детальніше про курс</a>
        </div>
      </article>`;
  }
