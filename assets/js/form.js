...
(function () {
  ...
  <div class="modal" role="dialog" aria-modal="true" aria-labelledby="fTitle">
    ...
    <form class="modal__body" id="leadForm" novalidate>
      ...
      <!-- ===== ПІДТВЕРДЖЕННЯ ===== -->
      <section class="fstep" data-step="ok">
        <div class="fdone">
          ...
          <h3>Дякуємо! Вашу заявку отримано</h3>
          <p>Місце попередньо заброньовано. Щоб оформити участь і, за необхідності, дані для БПР, заповніть додаткову інформацію.</p>
          <button type="button" class="btn btn--primary btn--block btn--lg" data-goto2>Продовжити оформлення</button>
          <button type="button" class="btn btn--ghost btn--block" data-close>Пізніше</button>
        </div>
      </section>

      <!-- ===== STEP 2 — ПОВНА РЕЄСТРАЦІЯ ===== -->
      <section class="fstep" data-step="2">
        <h3>Оформлення участі</h3>
        <p class="fstep__lead">Ці дані потрібні для сертифіката та нарахування балів БПР.</p>
        ...
        <button type="submit" class="btn btn--primary btn--block btn--lg" id="fullBtn">Завершити оформлення</button>
      </section>
  ...
  function validateFull() {
    ...
    if (nm.length < 5 || !nm.includes(' ')) { setErr('fullnameUa', true); ok = false; } else setErr('fullnameUa', false);
    ...
    if (form.bpr.value === 'Так') {
      [['fullnameEn', form.fullnameEn.value.trim().length > 2],
       ['dob', !!form.dob.value],
  ...
  qs('#leadBtn').addEventListener('click', async () => {
    ...
    /* для підбору курсу другий крок не потрібен */
  ...
  qsa('[data-goto2]', form).forEach(b => b.addEventListener('click', () => show(2)));
