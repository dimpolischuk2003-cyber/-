...
(function () {
  const { D, qs, qsa, MONTHS, MONTHS_GEN, parseDate, upcoming,
          findCourse, formatPrice, getAttribution, trackEvent,
          viberGroup } = window.V;

  let ctx = { course: null, leadType: 'course', leadSent: false, leadId: '' };

  /* Ідентифікатор заявки. Обидва кроки летять з ним, тому в таблиці
     доповнюється той самий рядок, а не створюється новий. */
  const newLeadId = () =>
    ...
    <form class="modal__body" id="leadForm" novalidate>
      ...
      <section class="fstep active" data-step="1">
        ...
        <div class="field hidden" data-field="course" id="fCoursePick">
          ...
          <p class="fcourse hidden" id="fCourse">
      ...
      <section class="fstep" data-step="final">
        <div class="fdone">
          ...
          <a class="btn btn--primary btn--block btn--lg hidden" id="viberBtn" href="#" rel="noopener">
            Перейти у Viber-групу курсу
          </a>
          <p class="fnote hidden" id="viberNote">
            Перенаправимо автоматично через <span id="viberCount">5</span> с
          </p>
          <button type="button" class="btn btn--ghost btn--block" data-close>Закрити</button>
        </div>
      </section>
    </form>
  </div>`;
  ...
  function payload(stage) {
    ...
    const c = findCourse(d.course) || ctx.course;
  ...
  /* ---------- підпис під селектом курсу ---------- */
  const courseSel = qs('[name=course]', form);

  function syncCourseInfo() {
    const card = qs('#fCourse');
    ...
    qs('#fCourseMeta').textContent =
      `${dt.getDate()} ${MONTHS_GEN[dt.getMonth()]} ${dt.getFullYear()} • ${c.city} • ${sp.name}`;
    card.classList.remove('hidden');
  }
  courseSel.addEventListener('change', () => { syncCourseInfo(); setErr('course', false); });
  ...
  qs('#leadBtn').addEventListener('click', async () => {
    ...
    /* людина залишила тільки контакти — це початок реєстрації */
  ...
  qs('#step2Btn').addEventListener('click', () => {
    if (!ctx.leadSent) { show(1); return; }   /* крок 1 обов'язковий */
  ...
  form.addEventListener('submit', async e => {
    e.preventDefault();
    if (!ctx.leadSent) { show(1); return; }   /* крок 1 обов'язковий */
    if (!validateStep2()) { show(2); return; } /* крок 2 обов'язковий */
    ...
    /* усі дані внесені — це повноцінний лід */
    trackEvent('Lead', {
      content_name: p.course_name, content_ids: [p.course_id],
      city: p.course_city, date: p.course_date
    });

    if (ctx.course && ctx.course.template === 'complexEndo') {
      qs('#finalText').textContent =
        `Дякуємо за реєстрацію на курс «${ctx.course.title}». Команда VSEODENT перевірить дані та зв’яжеться з вами щодо підтвердження участі.`;
    } else {
    ...
    /* перехід у viber-групу свого курсу й міста */
    const viber = viberGroup(ctx.course);
    const vbtn = qs('#viberBtn');
    if (viber) {
      vbtn.href = viber;
      vbtn.classList.remove('hidden');
      qs('#viberNote').classList.remove('hidden');
      let left = 5;
      const tick = qs('#viberCount');
      ...
      const timer = setInterval(() => {
        ...
        if (left <= 0) {
          clearInterval(timer);
          if (backdrop.classList.contains('open')) window.location.href = viber;
        }
      }, 1000);
      vbtn.addEventListener('click', () => clearInterval(timer));
      qsa('[data-close]', backdrop).forEach(b =>
        b.addEventListener('click', () => clearInterval(timer)));
    } else {
      vbtn.classList.add('hidden');
      qs('#viberNote').classList.add('hidden');
    }
  ...
  window.openLeadForm = (slug = '', type = 'course') => {
    ...
    ctx = { course: findCourse(slug) || null, leadType: type,
            leadSent: false, leadId: newLeadId() };
    const pick = qs('#fCoursePick');
    ...
    if (type === 'course_selection') {
      ...
      qs('#fCourse').classList.add('hidden');
      courseSel.value = '';
    } else {
      qs('#fTitle').textContent = 'Забронюйте місце на курсі';
      qs('.fstep__lead', form).textContent =
        'Перевірте курс і залиште контакти — команда VSEODENT зв’яжеться для підтвердження участі.';
      qs('#leadBtn').textContent = 'Забронювати місце';

      /* селект видно завжди: потрібний курс уже підставлений,
         але його можна змінити прямо у вікні */
      pick.classList.remove('hidden');
      courseSel.value = ctx.course ? ctx.course.slug : '';
      syncCourseInfo();
    }
