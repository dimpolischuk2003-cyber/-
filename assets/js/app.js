...
(function () {
  ...
  const viberGroup = (course) => {
    ...
    /* Складна ендодонтія — без автоматичного переходу у Viber.
       Участь доступна лише після ручної перевірки командою VSEODENT,
       тому після реєстрації користувач просто бачить success-screen. */
    if (course.template === 'complexEndo') return '';
  ...
  const speakerPhoto = (course) => {
    ...
    if (!sp) return '';
    // Для всіх курсів Ігоря використовуємо друге фото (боком),
    // бо воно коректніше кадрується на desktop і mobile.
    if (course.speaker === 'noienko' && sp.imageAlt) return sp.imageAlt;
    return (course.photoAlt && sp.imageAlt) ? sp.imageAlt : sp.image;
  };

  /* --- URL сторінки конкретного курсу -----------------------
     Vercel працює з cleanUrls, тому production URL — /course?slug=...
     Для GitHub Pages / file / localhost лишаємо course.html?slug=...
     Не використовуємо /kurs/:slug у кнопках, щоб уникнути 404 при деплої. */
  const courseUrl = (courseOrSlug) => {
    const raw = typeof courseOrSlug === 'string' ? courseOrSlug : (courseOrSlug && courseOrSlug.slug);
    if (!raw) return 'index.html#kursy';
    const slug = encodeURIComponent(raw);
    const staticPreview = location.protocol === 'file:' ||
      location.hostname.endsWith('github.io') ||
      location.hostname === 'localhost' ||
      location.hostname === '127.0.0.1';
    return staticPreview ? `course.html?slug=${slug}` : `/course?slug=${slug}`;
  };
  ...
  window.V = {
    ...
    resolvePricing, mainPrice, viberGroup, speakerPhoto, courseUrl,
