...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!slug) {
      const legacy = location.pathname.match(/\/kurs\/([^/?#]+)/);
    ...
    // Speaker-specific layout hooks. Avoid CSS :has()/src matching for critical mobile hero framing.
    document.body.classList.toggle('course-speaker-tarasovska', c.speaker === 'tarasovska');
    document.body.classList.toggle('course-speaker-noienko', c.speaker === 'noienko');
    document.body.classList.toggle('course-speaker-fedak', c.speaker === 'fedak');
    ...
    qs('#heroSpeakerImg').src = window.V.speakerPhoto(c);
    ...
    const media = t.media || { videos: [], photos: [] };
    window.renderVideos('#videos', { videos: media.videos });
    window.renderPhotos('#photos', media.photos);
    ...
    /* ================= 7. СПІКЕР ================= */
    qs('#spImg').src = window.V.speakerPhoto(c);
    qs('#spImg').alt = sp.name;
    qs('#spName').textContent = sp.name;
    if (sp.facts && sp.facts.length) {
      qs('#spFacts').innerHTML = sp.facts.map(f => {
        const title = f.url
          ? `<a class="fact__title-link" href="${esc(f.url)}" target="_blank" rel="noopener noreferrer">${esc(f.title)}<span class="fact__link-icon" aria-hidden="true">↗</span></a>`
          : `<b>${esc(f.title)}</b>`;
        return `<article class="fact">${title}<span>${esc(f.text)}</span></article>`;
      }).join('');
    } else kill('#spFacts');
    ...
    if (others.length) {
      qs('#otherDates').innerHTML = others.map(o => {
        ...
        return `<article class="dcard">
          ...
          <a class="btn btn--ghost btn--sm btn--block" href="${esc(courseUrl(o))}">Переглянути курс</a>
        </article>`;
      }).join('');
    } else kill('#secOther');
    ...
    if (rest.length) {
      qs('#otherCourses').innerHTML = rest.map(o => {
        ...
        return `<a class="mini" href="${esc(courseUrl(o))}">
